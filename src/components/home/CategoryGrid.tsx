          ...
          {categories.slice(0, 4).map((cat) => (
            ...
            >
              ...
              <div className="absolute inset-0 p-5 md:p-7 flex flex-col justify-end text-sand">
                <h3 className="font-serif text-xl md:text-2xl leading-tight">{cat.name}</h3>
                <span className="mt-2 text-[10px] uppercase tracking-wider-2 text-gold group-hover:text-sand transition flex items-center gap-1">
        ...
        <div className="mt-3 md:mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
          {categories.slice(4).map((cat) => (
            <Link
              key={cat.slug}
              href={`/kategori/${cat.slug}`}
              className="group block relative aspect-[16/9] overflow-hidden bg-ink"
            >
              <Image
                src={cat.hero || "/images/products/essence-set-cream.jpg"}
                alt={cat.name}
                fill
              ...
              <div className="absolute inset-0 p-7 flex flex-col justify-end text-sand">
                <h3 className="font-serif text-xl leading-tight">{cat.name}</h3>
                <span className="mt-2 text-[10px] uppercase tracking-wider-2 text-gold flex items-center gap-1">
                  Keşfet →
                </span>
              </div>
            </Link>
